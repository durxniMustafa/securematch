...
| **Opposite swing too small** | The head returns only partway to neutral so the second crossing never reaches the threshold. | Lower `pitchThresh`, shorten `swingMinMs`, or set `oppSwingRatio` (e.g. `0.4`) so a partial return swing counts. |
